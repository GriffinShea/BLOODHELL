...
def game_loop_update(rotating_line, circle_hitbox):

	# increase the angle of the rotating line
	rotating_line["ang"] = (rotating_line["ang"] + 5)
...
def game_loop_render(rotating_line, circle_hitbox, window_sfc):
...
def main():
	...
	rotating_line["len"] = [ (0.00, 0.50) ] 			   # the "length" intervals that specify the gap(s)
	...
	while not closed_flag:
		...
		rotating_line, circle_hitbox = game_loop_update(rotating_line, circle_hitbox) 
		
		#####################################################################################################
		# this is the "render" phase of the game loop, where a representation of the game world is displayed
		#####################################################################################################

		game_loop_render(rotating_line, circle_hitbox, window_sfc)
